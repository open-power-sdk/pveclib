...
/** \brief Vector Extract Quadword High-Order
 *
 *  Extract a Quadword of elements from a double Quadword (vra||vrb)
 ...
 *  The index (gprc, bits [59-63]) in the range 0-16 selects a
 *  total of 16 bytes. The resulting quadword is returned in vector.
 *
 *  \note There is no equivalent to this operation in the PVIPR or
 *  PowerISA but this operation provides a logical extension to the
 *  vec_extracth intrinsic.
 *  \sa vec_extracth_byte(), vec_extracth_DW(), vec_extracth_HW,
 *  vec_extracth_word().
 *  \note The results are boundedly undefined if the index is greater
 *  then 16.
 *  \sa vec_sldox(), vec_srdox().
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   4   | 2/cycle  |
 ...
 *  @return Indexed elements returned in a vector.
 *
 *  \showrefby
 */
static inline vui8_t
vec_extracth_QW (vui8_t vra, vui8_t vrb, int gprc)
{
  vui8_t result;
#if (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
 ...
 *  \note There is no equivalent to this operation in the PVIPR or
 *  PowerISA but this operation provides a logical extension to the
 *  vec_extractl intrinsic.
 *  \sa vec_extractl_byte(), vec_extractl_DW(), vec_extractl_HW,
 *  vec_extractl_word().
 *  \note The results are boundedly undefined if the index is greater
 *  then 16.
 *  \sa vec_sldox(), vec_srdox().
 ...
 */
static inline vui8_t
vec_extractl_QW (vui8_t vra, vui8_t vrb, int gprc)
{
  vui8_t result;
#if (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
 ...
 *  (vra||vrb).  The index is from bits 60:63 of gpra.
 *
 ...
 *  \sa vec_extracth_QW(), vec_extractl_QW().
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   4   | 2/cycle  |
 ...
 */
static inline vui8_t
vec_sldox (vui8_t vra, vui8_t vrb, int gpra)
{
  return vec_sldox_PWR9 (vra, vrb, gpra);
}
 ...
 */
static inline vui8_t
vec_srdox (vui8_t vra, vui8_t vrb, int gpra)
{
  return vec_srdox_PWR9 (vra, vrb, gpra);
}
