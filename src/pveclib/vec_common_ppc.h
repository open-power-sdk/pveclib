...
/** \brief Load Vector for Shift Left Indexed.
 *
 *  Load a Permute Control Vector for extracting a quadword from a
 *  double quadword starting at a specified index.
 *  The index (0-15) selects the starting (high order) Octet of
 *  16 contiguous Octets from a pair of vectors (32 Octets).
 *  This can be used with Vector Permute to:
 *  - Align a sequence of 16 bytes spanning adjacent quadwords
 *    (from big-endian storage).
 *  - Extract a vector from a double quadword (VR pair).
 *  - Shift left double by octet
 *  - Rotate left by octet
 *
 *  \note This operation avoids the compiler warning generated by the
 *  vec_lvsl intrinsic for LE targets.
 *  This allows the instruction to be used for extended operations.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   2   | 2/cycle  |
 ...
 *  @param gprb Base address of array element
 *  @return Permute Control Vector for Shift Left Double Octet.
 *
 *  \showrefby
 */
static inline vui8_t
vec_lvsl_PWR7(int gpra, unsigned char * gprb)
{
  vui8_t pcv;
#if (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
  __asm__(
...
/** \brief Load Vector for Shift Right Indexed.
 *
 *  Load a Permute Control Vector for extracting a quadword from a
 *  double quadword starting at a specified index.
 *  The index (0-15) selects the starting (low order) Octet of
 *  16 contiguous Octets from a pair of vectors (32 Octets).
 *  This can be used with Vector Permute to:
 *  - Extract a vector from a double quadword (VR pair).
 *  - Shift right double by octet
 *
 *  \note This operation avoids the compiler warning generated by the
 *  vec_lvsr intrinsic for LE targets.
 *  This allows the instruction to be used for extended operations.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 *  @param gpra Integer index (0-15)
 *  @param gprb Base address of array element
 *  @return Permute Control Vector for Shift Right Double Octet.
 *
 *  \showrefby
 */
static inline vui8_t
vec_lvsr_PWR7(int gpra, unsigned char * gprb)
{
  vui8_t pcv;
#if (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
  __asm__(
...
/** \brief Vector Shift Left Double by Octet using VSR specified shift.
 *
 ...
 */
static inline vui8_t
vec_sldov_PWR7 (vui8_t vrw, vui8_t vrx, vui8_t vrb)
{
  vui8_t result, vt1, vt2, vt3, vbs, v_3;
  ...
  // Convert Octet index for bit shift count
  v_3= vec_splat_u8(3);
  vbs = vec_vslb (vrb, v_3);
  vt1 = vec_slo (vrw, vbs);
...
/** \brief Vector Shift Left Double by Octet using GPR specified index.
 *
 *  Vector Shift Left double Quadword Octex 0-15 bytes.
 *  Return a vector unsigned char that is the left most 16-bytes after
 *  shifting left 0-15 bytes of the 32-byte double vector
 *  (vra||vrx).  The index is from bits 60:63 of gpra.
 *
 *  This an extender vector operation using lvsl/vperm.
 *  The index (0-15) selects the starting (high order) Octet of
 *  16 contiguous Octets from a pair of vectors (32 Octets).
 *  This can be used with Vector Permute to:
 *  - Align a sequence of 16 bytes spanning adjacent quadwords
 ...
 *
 *  \note This operation avoids the compiler warning generated by the
 *  vec_lvsl intrinsic and bi-endian transforms for vec_perm
 *  associated with LE targets
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   4   | 2/cycle  |
 ...
 *  |power10  | 6 - 8 | 4/cycle  |
 *
 *  @param vra upper 16-octets of the 32-byte double vector.
 *  @param vrb lower 16-octets of the 32-byte double vector.
 *  @param gpra Integer Left Octet index (0-15)
 *  @return Vector of Octets (index:index+15)
 *
 *  \showrefby
 */
static inline vui8_t
vec_sldox_PWR8 (vui8_t vra, vui8_t vrb, int gpra)
{
  vui8_t pcv;
...
/** \copybrief vec_sldox_PWR8
 *  \note Support xxperm with access to all 64 VSX registers.
 *  \sa vec_sldox_PWR8 for details.
 *  \showrefby
 * */
static inline vui8_t
vec_sldox_PWR9 (vui8_t vra, vui8_t vrb, int gpra)
{
  vui8_t pcv;
  pcv = vec_vgenpcvsldx_PWR7 (gpra);
  return vec_vperm_PWR9 (vra, vrb, pcv);
}

/** \brief Vector Shift Right Double by Octet using GPR specified index.
 *
 *  Vector Shift Left double Quadword Octex 0-15 bytes.
 *  Return a vector unsigned char that is the left most 16-bytes after
 *  shifting left 0-15 bytes of the 32-byte double vector
 *  (vra||vrx).  The index is from bits 60:63 of gpra.
 *
 *  This an extender vector operation using lvsr/vperm.
 *  The index (0-15) selects the starting (low order) Octet of
 *  16 contiguous Octets from a pair of vectors (32 Octets).
 *  This can be used with Vector Permute to:
 *  - Extract a vector from a double quadword (VR pair).
 ...
 *
 *  \note This operation avoids the compiler warning generated by the
 *  vec_lvsr intrinsic and bi-endian transforms for vec_perm
 *  associated with LE targets
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 *  |power10  | 6 - 8 | 4/cycle  |
 *
 *  @param vra upper 128-bits of the 256-bit double vector.
 *  @param vrb lower 128-bits of the 256-bit double vector.
 *  @param gpra Integer index (0-15)
 *  @return Vector of Octets (16-index:31-index)
 *
 *  \showrefby
 */
static inline vui8_t
vec_srdox_PWR8 (vui8_t vra, vui8_t vrb, int gpra)
{
  vui8_t pcv;
...
/** \copybrief vec_srdox_PWR8
 *  \note Support xxperm with access to all 64 VSX registers.
 *  \sa vec_srdox_PWR8 for details.
 *  \showrefby
 * */
static inline vui8_t
vec_srdox_PWR9 (vui8_t vra, vui8_t vrb, int gpra)
{
  vui8_t pcv;
  pcv = vec_vgenpcvsrdx_PWR7 (gpra);
  return vec_vperm_PWR9 (vra, vrb, pcv);
 ...
 */
static inline vui8_t
vec_sldq_PWR10 (vui8_t vrw, vui8_t vrx, vui8_t vrb)
{
  vui8_t result;
#if defined (_ARCH_PWR10)
  vui8_t vt1, vt2, vt3, vbs;
  vbs = (vui8_t) vec_xxswapd_PWR7 ((vui64_t) vrb);
  vt3 = (vui8_t) vec_neg ((vi64_t) vbs);
  vt1 = (vui8_t) vec_vslq_PWR10 ((vui128_t) vrw, vbs);
  vt2 = (vui8_t) vec_vsrq_PWR10 ((vui128_t) vrx, vt3);
  result = vec_or (vt1, vt2);
#else
  result = vec_sldq_PWR7 (vrw, vrx, vrb);
#endif
  return result;
}
...
/** \brief Vector Extract Double Doubleword to VSR using GPR-specified Left-Index
 *
 *  Vector Extract a Doubleword element from a double Quadword
 *  (32 octets), starting from the Octet specified by the Left-index
 *  and continuing for a total of 8 Octets.
 *  The index (gprc, bits [59-63]) selects the starting (high order)
 *  Octet of 8 contiguous Octets.
 *  The resulting doubleword is returned in the high-order doubleword
 *  of a vector. The low-order doubleword is set to zero.
 *
 *  \note If the index is greater then 24 the result is boundedly undefined.
 *  \note This operation provides only the function of vextddvlx and
 *  is not bi-endian enabled.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   | 6 - 8 | 2/cycle  |
 ...
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param gprc Integer index (0-24)
 *  @return Vector with Octets (index:index+7) in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
vec_vextddvlx_PWR7 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
  const vui8_t zero = vec_splat_u8(0);
  vui8_t pcv, res;
  ...
  /* Enable VMX register usage only. */
 ...
 * */
static inline vui64_t
vec_vextddvlx_PWR9 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
...
/** \copybrief vec_vextddvlx_PWR7
 *  \note Generate vextddvlx instruction for _ARCH_PWR10.
 *  Else generate equivalent function for PWR7/8/9.
 *  \sa vec_vextddvlx_PWR7 for details.
 *  \showrefby
 */
static inline vui64_t
vec_vextddvlx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
...
#endif
#else
  result = vec_vextddvlx_PWR9 (vra, vrb, gprc);
#endif
  return result;
}

/** \brief Vector Extract Double Doubleword to VSR using GPR-specified Right-Index
 *
 *  Vector Extract a Doubleword element from a double Quadword
 *  (32 Octets), starting from the Octet specified by the right-index
 *  and continuing left, for a total of 8 Octets.
 *  The index (gprc, bits [59-63]) selects the starting (low order)
 *  Octet of 8 contiguous Octets.
 *  The resulting doubleword is returned in the high-order doubleword
 *  of a vector. The low-order doubleword is set to zero.
 *
 ...
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param gprc Integer index (0-24)
 *  @return Vector with Octets (24-index:31-index) in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
vec_vextddvrx_PWR7 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
  const vui8_t zero = vec_splat_u8(0);
  vui8_t pcv, res;
...
#else
  // shift low-DW into high-DW with trailing zeros.
  result =  (vui64_t) vec_sld (res, zero, 8);
#endif
  return result;
}
 ...
 * */
static inline vui64_t
vec_vextddvrx_PWR9 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined(_ARCH_PWR9) && defined (__VSX__)
  const vui8_t zero = vec_splat_u8(0);
...
/** \copybrief vec_vextddvrx_PWR7
 *  \note Generate vextddvrx instruction for _ARCH_PWR10.
 *  Else generate equivalent function for PWR7/8/9.
 *  \sa vec_vextddvrx_PWR7 for details.
 *  \showrefby
 */
static inline vui64_t
vec_vextddvrx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
      result = vec_extractl ((vui64_t) vrb, (vui64_t) vra, gprc);
#else
  __asm__(
      "vextddvrx %0,%1,%2,%3;\n"
      : "=v" (result)
...
#endif
#else
  result = vec_vextddvrx_PWR9 (vra, vrb, gprc);
#endif
  return result;
}

/** \brief Vector Extract Double Quadword to VSR using GPR-specified Left-Index
 *
 *  Vector Extract a Quadword element from a double Quadword starting
 *  from the Octet specified by the Left-index and continuing for a
 *  total of 16 Octets.
 *  The index (RC, bits [59-63]) selects the starting (high order)
 *  Octet of 16 contiguous Octets from a pair (VRA||VRB) of vectors
 *  (32 Octets).
 *  If the index is greater then 16 the result is boundedly undefined.
 *
 *  This operation can be used to:
 *  - Align a sequence of 16 bytes spanning adjacent quadwords
 *    (from big-endian storage).
 *  - Extract a quadword element from a double quadword (VR pair).
 *  - Shift left double by octet indexed.
 *  - Rotate left by octet indexed.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   4   | 2/cycle  |
 ...
 *  @param vra Quadword containing Octets 0-15 of the source.
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param gprc Integer index (0-16)
 *  @return Vector of Octets (index:index+15)
 *
 *  \showrefby
 */
static inline vui8_t
vec_vextdqvlx_PWR8 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui8_t pcv, result;
  pcv = vec_vgenpcvsldx_PWR7 (gprc);
  if (__builtin_expect (gprc < 16, 1))
...
/** \copybrief vec_vextdqvlx_PWR8
 *  \note Support xxperm with access to all 64 VSX registers.
 *  \sa vec_vextdqvlx_PWR8 for details.
 *  \showrefby
 * */
static inline vui8_t
vec_vextdqvlx_PWR9 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui8_t pcv, result;
  pcv = vec_vgenpcvsldx_PWR7 (gprc);
  if (__builtin_expect (gprc < 16, 1))
    result = vec_vperm_PWR9 (vra, vrb, pcv);
  else // if (gprc == 16) result = vrb else undefined
    result = vrb;
  return result;
}

/** \brief Vector Extract Double Quadword to VSR using GPR-specified Right-Index
 *
 *  Vector Extract a Quadword element from a double Quadword starting
 *  from the Octet specified by the Right-index and continuing left
 *  for a total of 16 Octets.
 *  The index (RC, bits [59-63]) selects the starting (low order)
 *  Octet of 16 contiguous Octets from a pair (VRA||VRB) of vectors
 *  (32 Octets).
 *  If the index is greater then 16 the result is boundedly undefined.
 *
 ...
 *    (from big-endian storage).
 *  - Extract a quadword element from a double quadword (VR pair).
 *  - Shift right double by octet indexed.
 *  - Rotate right by octet indexed.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   4   | 2/cycle  |
 ...
 */
static inline vui8_t
vec_vextdqvrx_PWR8 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui8_t pcv, result;
  pcv = vec_vgenpcvsrdx_PWR7 (gprc);
  if (__builtin_expect (gprc < 16, 1))
...
/** \copybrief vec_vextdqvrx_PWR8
 *  \note Support xxperm with access to all 64 VSX registers.
 *  \sa vec_vextdqvrx_PWR8 for details.
 *  \showrefby
 * */
static inline vui8_t
vec_vextdqvrx_PWR9 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui8_t pcv, result;
  pcv = vec_vgenpcvsrdx_PWR7 (gprc);
  if (__builtin_expect (gprc < 16, 1))
...
/** \brief Vector Extract Double Unsigned Byte to VSR using GPR-specified Left-Index
 *
 *  Vector Extract a Byte element (Octet) from a double Quadword (32 bytes),
 ...
 *
 *  \note This operation provides only the function of vextdubvlx and
 *  is not bi-endian enabled.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 */
static inline vui64_t
vec_vextdubvlx_PWR7 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
  vui8_t pcv, res;
  const vui8_t zero = vec_splat_u8(0);
 ...
 */
static inline vui64_t
vec_vextdubvlx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
...
/** \brief Vector Extract Double Unsigned Byte to VSR using GPR-specified Right-Index
 *
 *  Vector Extract a Byte element (Octet) from a double Quadword (32 bytes),
 ...
 *  of a vector. The low-order doubleword is set to zero.
 *
 *  \note This operation provides only the function of vextdubvrx and
 *  is not bi-endian enabled.
 *
 *
 *  |processor|Latency|Throughput|
 ...
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param gprc Integer index (0-31)
 *  @return Vector with Octet (31-index) zero extended to doubleword in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
vec_vextdubvrx_PWR7 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
  vui8_t pcv, res;
  const vui8_t zero = vec_splat_u8(0);
  ...
  // rotate result into low-byte of high DW
  result = (vui64_t) vec_sld (res,res, 9);
  return result;
}
 ...
 */
static inline vui64_t
vec_vextdubvrx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
...
/** \brief Vector Extract Double Halfword to VSR using GPR-specified Left-Index
 *
 *  Vector Extract a halfword element from a double Quadword
 *  (32 octets), starting from the Octet specified by the Left-index
 *  and continuing right for a total of 2 Octets.
 *  The index (gprc, bits [59-63]) selects the starting (high order)
 *  Octet of 2 contiguous Octets.
 *  The selected halfword is zero extended to doubleword.
 *  The resulting doubleword is returned in the high-order doubleword
 *  of a vector. The low-order doubleword is set to zero.
 *
 *  \note If the index is greater then 30 the result is boundedly undefined.
 *  \note This operation provides only the function of vextduhvlx and
 *  is not bi-endian enabled.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 *  @return Vector with Octets (index:index+1) in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
vec_vextduhvlx_PWR7 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
  vui8_t pcv, res;
  const vui8_t zero = vec_splat_u8(0);
  ...
  // shift high-HW in to low-HW with leading zeros.
 ...
 */
static inline vui64_t
vec_vextduhvlx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
      result = vec_extracth ((vui16_t) vrb, (vui16_t) vra, gprc);
#else
  __asm__(
      "vextduhvlx %0,%1,%2,%3;\n"
      : "=v" (result)
...
/** \brief Vector Extract Double Halfword to VSR using GPR-specified Right-Index
 *
 *  Vector Extract aHalfword element from a double Quadword
 *  (32 Octets), starting from the Octet specified by the right-index
 *  and continuing left, for a total of 2 Octets.
 *  The index (gprc, bits [59-63]) selects the starting (low order)
 *  Octet of 2 contiguous Octets.
 *  The selected halfword is zero extended to doubleword.
 *  The resulting doubleword is returned in the high-order doubleword
 ...
 *
 *  \note If the index is greater then 30 the result is boundedly undefined.
 *  \note This operation provides only the function of vextduhvrx and
 *  is not bi-endian enabled.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param gprc Integer index (0-30)
 *  @return Vector with Octets (30-index:31-index) in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
vec_vextduhvrx_PWR7 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
  vui8_t pcv, res;
  const vui8_t zero = vec_splat_u8(0);
  ...
  // shift low-HW into high-HW with trailing zeros.
  res = vec_sld (res, zero, 14);
  // rotate result into low-HW of high DW
  result = (vui64_t) vec_sld (res,res, 10);
  return result;
}
 ...
 */
static inline vui64_t
vec_vextduhvrx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
      result = vec_extractl ((vui16_t) vrb, (vui16_t) vra, gprc);
#else
  __asm__(
      "vextduhvrx %0,%1,%2,%3;\n"
      : "=v" (result)
...
/** \brief Vector Extract Double Word to VSR using GPR-specified Left-Index
 *
 *  Vector Extract a word element from a double Quadword
 *  (32 octets), starting from the Octet specified by the Left-index
 *  and continuing right for a total of 4 Octets.
 *  The index (gprc, bits [59-63]) selects the starting (high order)
 *  Octet of 4 contiguous Octets.
 *  The selected word is zero extended to doubleword.
 *  The resulting doubleword is returned in the high-order doubleword
 *  of a vector. The low-order doubleword is set to zero.
 *
 *  \note If the index is greater then 28 the result is boundedly undefined.
 *  \note This operation provides only the function of vextduwvlx and
 *  is not bi-endian enabled.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 *  @return Vector with Octets (index:index+3) in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
...
/** \copybrief vec_vextduwvlx_PWR7
 *  \note Generate vextduwvlx instruction for _ARCH_PWR10.
 *  Else generate equivalent function for PWR7/8/9.
 *  \sa vec_vextduwvlx_PWR7 for details.
 *  \showrefby
 */
static inline vui64_t
vec_vextduwvlx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
...
#endif
#else
  result = vec_vextduwvlx_PWR9 (vra, vrb, gprc);
#endif
  return result;
}


/** \brief Vector Extract Double Word to VSR using GPR-specified Right-Index
 *
 *  Vector Extract a Word element from a double Quadword
 *  (32 Octets), starting from the Octet specified by the right-index
 *  and continuing left, for a total of 4 Octets.
 *  The index (gprc, bits [59-63]) selects the starting (low order)
 *  Octet of 4 contiguous Octets.
 *  The selected word is zero extended to doubleword.
 *  The resulting doubleword is returned in the high-order doubleword
 ...
 *
 *  \note If the index is greater then 28 the result is boundedly undefined.
 *  \note This operation provides only the function of vextduwvrx and
 *  is not bi-endian enabled.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 ...
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param gprc Integer index (0-28)
 *  @return Vector with Octets (28-index:31-index) in DW 0.
 *
 *  \showrefby
 */
static inline vui64_t
...
/** \copybrief vec_vextduwvrx_PWR7
 *  \note Generate vextduwvrx instruction for _ARCH_PWR10.
 *  Else generate equivalent function for PWR7/8/9.
 *  \sa vec_vextduwvrx_PWR7 for details.
 *  \showrefby
 */
static inline vui64_t
vec_vextduwvrx_PWR10 (vui8_t vra, vui8_t vrb, int gprc)
{
  vui64_t result;
#if defined (_ARCH_PWR10)  && (__GNUC__ >= 10)
#if defined (vec_extracth) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
 ...
 *  - Rotate left by octet indexed.
 *
 *  \note This operation uses the lvsl instruction but avoids the
 *  compiler warning generated by the vec_lvsl intrinsic for LE
 *  targets.
 *  This allows the instruction to be used as an extended operation
 *  <I>Vector Generate PCV from SLDO (GPR) Index</I>.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   2   | 2/cycle  |
...
/** \brief Vector Generate PCV from SRDO Index.
 *
 *  Vector Generate a Permute Control Vector for Shift Right Double
 *  Octet from GPR Index.
 *  The index (0-15) selects the starting (low order) Octet of
 *  16 contiguous Octets from a pair of vectors (32 Octets).
 *  This can be used with Vector Permute to:
 ...
 *  - Shift right double by octet
 *
 *  \note This operation uses the lvsr instruction but avoids the
 *  compiler warning generated by the vec_lvsr intrinsic for LE
 *  targets.
 *  This allows the instruction to be used as an extended operation
 *  <I>Vector Generate PCV from SRDO (GPR) Index</I>.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   2   | 2/cycle  |
 ...
 *
 *  Permute Octets from a Double Quadword source using a Permute Control Vector (PCV).
 *
 *  The result is a Quadword where each Octet is selected from one of the 32-Octets
 *  based on the index from the corresponding PCV Octet.
 *
 *  \note The operation generates the raw vperm instruction for Big-Endian.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   2   | 2/cycle  |
 ...
 *  @param vra Quadword containing Octets 0-15 of the source.
 *  @param vrb Quadword containing Octets 16-31 of the source.
 *  @param pcv Permute Control Vector where each Octet contains an index in bits 3-7.
 *  @return The permuted vector contents of the Double Quadword source.
 *
 *  \showrefby
 */
static inline vui8_t
vec_vperm_PWR7 (vui8_t vra, vui8_t vrb, vui8_t pcv)
{
  vui8_t vrt;
  /* Assume P7 is BE only and vec_perm intrinsic will not trigger
   * LE transform. */
  vrt = vec_perm (vra, vrb, pcv);
  return vrt;
}
 ...
 *  based on the index from the corresponding PCV Octet.
 *
 *  \note The operation generates the raw vperm instruction without any
 *  Little Endian transforms. This works for Big/Little-endian.
 *
 *  |processor|Latency|Throughput|
 *  |--------:|:-----:|:---------|
 *  |power7   |   2   | 2/cycle  |
...
vec_vperm_PWR8 (vui8_t vra, vui8_t vrb, vui8_t pcv)
{
  ...
  __asm__(
      ...
      : );
#else
  /* Else P8/BE and vec_perm intrinsic will not trigger
   * LE transform. */
  vrt = vec_vperm_PWR7 (vra, vrb, pcv);
#endif
  return vrt;
}


/** \copybrief vec_vperm_PWR8
 *  \note Support xxperm with access to all 64 VSX registers.
 *  If __VSX__ enabled.
 *  \sa vec_vperm_PWR8 for details.
 *  \showrefby
 * */
static inline vui8_t
vec_vperm_PWR9 (vui8_t vra, vui8_t vrb, vui8_t pcv)
{
...
#if (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
#if defined(_ARCH_PWR9) && defined (__VSX__)
  /* ISA 3.0 add VSX enable XX3 form with operand overwritten by
   * result. Gives access to 64 VSRs but may require addition move. */
  __asm__(
      "xxperm %x0,%x1,%x2;"
      : "+wa" (vrb)
      : "wa" (vra), "wa" (pcv)
  ...
  vrt = vrb;
#else
  vrt = vec_vperm_PWR8 (vra, vrb, pcv);
#endif
#else
  /* Else P7/8 and BE so vec_perm intrinsic will not trigger
   * LE transform. */
  vrt = vec_vperm_PWR7 (vra, vrb, pcv);
#endif
  return vrt;
...
/** \brief VSX Vector splat doubleword.
 ...
 */
static inline vui64_t
vec_xxspltd_PWR7 (vui64_t vra, const int ctl)
{
  vui64_t result;
  ...
  switch (ctl & 1)
    ...
    case 0:
      /*
       ...
       result[0] = vra[0];
       */
      result = vec_xxpermdi_PWR7 (vra, vra, 0);
      break;
    case 1:
      /*
       result[1] = vra[1];
       result[0] = vra[1];
       */
      result = vec_xxpermdi_PWR7 (vra, vra, 3);
      break;
    }
...
/** \brief VSX Vector doubleword swap.
...
vec_xxswapd_PWR7 (vui64_t vra)
{
  ...
  /*
   ...
   result[0] = vra[1];
   */
  result = vec_xxpermdi_PWR7 (vra, vra, 2);
